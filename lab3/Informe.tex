...
Para esta actividad, se incorporó una fotoresistencia al circuito que ya se venía trabajando. Conectamos una pata del componente a los 5V y la otra al pin analógico A0 del Arduino, colocando también una resistencia de protección que va hacia tierra (GND). De esta forma, se midió cómo cambia el voltaje dependiendo de la cantidad de luz que recibe el sensor. El diseño del circuito en Fritzing se puede ver en la Figura~\ref{fig:diseno_act2}, y el montaje real armado en el protoboard se muestra en la Figura~\ref{fig:circuito_act2}.
...
\pagebreak
En la función \texttt{setup()} del código, se creó un nuevo gráfico para ir dibujando la línea con los valores de la luz en el tiempo (C\'{o}digo~\ref{lst:grafica_fotoresistencia}).
...
\pagebreak
Luego, dentro de la función \texttt{draw()}, se usó \texttt{arduino.analogRead(0)} para leer el valor del pin \texttt{A0} a través de Firmata, mostrar el valor numérico en el panel de texto y actualizar el gráfico de línea (C\'{o}digo~\ref{lst:lectura_pin}).
...
Se agreg\'{o} un servo motor al circuito conect\'{a}ndolo al pin digital 10. Con
