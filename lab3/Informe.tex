...
Para cargarlo, se fue a \emph{Archivo} $\to$ \emph{Ejemplos} $\to$ \emph{Firmata} $\to$
\texttt{StandardFirmata} en el IDE y se subi\'{o} a la placa por USB. Con eso el Arduino
queda escuchando comandos por el puerto serie a 57\,600~baud y Processing puede
...
Los tres sliders se crearon con \texttt{cp5.addSlider()} con rango de 0 a 255, que es el
...
\textit{Esta secci\'{o}n corresponde a la participante Samantha Ram\'{i}rez y est\'{a}
...
Se agreg\'{o} un servo motor al circuito connect\'{a}ndolo al pin digital 10. En modo
...
\begin{figure}[ht!]
    \centering
    \includegraphics[width=0.8\linewidth]{img/resultado_act3_servo.png}
